...
import { CurrentWeatherModel, HourlyWeatherModel, SettingsModel } from '../../models'
...
export const Hourly = ({ settings, data, clickHandler }: HourlyProps) => {
  const [activeIndex, setActiveIndex] = useState(data && data.hourly[0] ? data.hourly[0].dt : 0)
        ...
        <ScrollContainer>
          {data.hourly.map((h) => (
            <div
              key={h.dt}
              className={
                activeIndex === h.dt ? 'hourly-item-container active' : 'hourly-item-container'
              }
